function jsonTimes(data){
    ...
    while(true){
        ...
        if(año.getDate() == 13 && año.getDay() == 5){
            obj.time += 1;
            obj.date += año.getMonth()+"/"+año.getDate()+"/"+año.getFullYear()+" ";
        }
...
function toLazyHuman(hasta, desde){
    ...
    if(año > 0 || año < 0){
        ...
        return closeTo(año,signo, "years");
    }
    if(mes > 0 || mes < 0){
        ...
        return closeTo(mes,signo, "months");    
    }
    if(dias > 0 || dias < 0){
        ...
        return closeTo(dias,signo, "days");
    }
    if(horas > 0 || horas < 0){
        // sin terminar, condicional si es menos de 10 agregale un 0
        var signo = horas < 0 ? "ago" : "in";
        minutos = Math.trunc(minutos * 1.6);
        minutos = (minutos < 50) ? "0"+ minutos : minutos;
        horas = Math.abs(Math.round(horas+"."+(minutos)));
        return closeTo(horas,signo, "hours");
    }
    if(minutos > 0 || minutos < 0){
        // sin terminar, condicional si es menos de 10 agregale un 0 y quitar decimales
        var signo = minutos < 0 ? "ago" : "in";
        segundos = Math.trunc(segundos * 1.6);
        segundos = (segundos < 50) ? "0"+ segundos : segundos;
        minutos = Math.abs(Math.round(minutos+"."+(segundos)));
        return closeTo(minutos,signo, "minutes");
    }
    if(segundos > 0 || segundos < 0){
        // sin terminar
        var signo = segundos < 0 ? "ago" : "in";
        segundos = Math.abs(Math.round(segundos+"."+(milisegundos)));
        return closeTo(segundos,signo, "seconds");
    }
...
// var fromDate = new Date("2019-08-31T12:01:00.000")
