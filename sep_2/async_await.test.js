...
const { get, post, request } = require('./async_await.viridianad.js');
...
describe('manipulations with await', () => {
  beforeEach(() => xhrMock.setup())
  afterEach(() => xhrMock.teardown())
  ...
  test('create a post', async () => {
    ...
    xhrMock.get(api.userGet.url, (req, res) => {
      return res.status(200).body(JSON.stringify(api.userGet.body))
    })

    xhrMock.post(api.postsPost.url, (req, res) => {
      expect(req.body()).toBe(JSON.stringify(api.postsPost.requestBody))
      return res.status(200).body(JSON.stringify(api.postsPost.body))
    })

    xhrMock.put(api.likePut.url, (req, res) => {
      expect(req.body()).toBe(JSON.stringify(api.likePut.requestBody))
      return res.status(200).body(JSON.stringify(api.likePut.body))
    })
    ...
    const postPost = JSON.parse(postData);
  ...
  test('like a post', async () => {
    ...
    const postPost = JSON.parse(postData);
    const likePost = await put(`/posts/${postPost.id}/like`, {
