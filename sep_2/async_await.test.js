...
const { get, post, put, request } = require('../aug_29/simple_xhr_hectors');
...
describe('manipulations with async-await', () => {
  ...
  test('create a post', async () => {
    ...
    const postPostedData = await post('/posts', {
      userId: user.id,
      content: 'This is my first post after being excomunicato',
    });
    const postPosted = JSON.parse(postPostedData);
  ...
  test('like a post', async () => {
    ...
    const postLikedData = await put(`/posts/${postPosted.id}/like`, {
