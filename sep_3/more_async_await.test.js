const { findHero,findSeries } = require('./more_async_await_luisg.js');
...
describe('Find Super heroes', () => {
  ...
  test('Thor', () => {
    ...
    return findHero('Thor')
      .then(thor => {
        expect(JSON.parse(thor).data.results[0].description).toBe("As the Norse God of thunder and lightning, Thor wields one of the greatest weapons ever made, the enchanted hammer Mjolnir. While others have described Thor as an over-muscled, oafish imbecile, he's quite smart and compassionate.  He's self-assured, and he would never, ever stop fighting for a worthwhile cause.");
      })
  });
});
...
describe('Find Series', () => {
  test('Avengers', async () => {
    let avengers = await findSeries('Avengers', { startYear: 2016 });
    avengers = JSON.parse(avengers).data.results[0];
