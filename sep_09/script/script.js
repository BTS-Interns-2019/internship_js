...
// console.log(tablero.board[150].posicion);
...
//    tablero.board = next.board;
  ...
  board();
    ...
    for (let i = 0; i < 200000; i++) {
       let state = tablero.board[i].estado;

       let neighbors = countNeighbors(i);

       if (state == 0 && neighbors == 3) {
         next.board[i].estado = 1;
       } else if (state == 1 && (neighbors < 2 || neighbors > 3)) {
         next.board[i].estado = 0;
       } else {
         next.board[i].estado = state;
       }
     }
   ...
   tablero.board = next.board;
      ...
      let sum = 0;
      if(tablero.board[x-500] == 1 )sum+=1;
      if(tablero.board[x-501] == 1 )sum+=1;
      if(tablero.board[x-499] == 1 )sum+=1;
      if(tablero.board[x+499] == 1 )sum+=1;
      if(tablero.board[x+500] == 1 )sum+=1;
      if(tablero.board[x+501] == 1 )sum+=1;
