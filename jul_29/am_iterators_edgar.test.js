...
test ('6 argumentos', () => {
    expect (foreach([10,11,23,21,56,98])).toBe(undefined)
})

test ('4 argumentos', () => {
    expect (foreach(["hey",2,5,"lol"])).toBe(undefined)
})

test ('3 argumentos', () => {
    expect (foreach(["hjj", "cmn", "jdf"])).toBe(undefined)
})
...
test ('3 argumentos', () => {
    expect (reducesright(["hjj","cmn","jdf"], "hjj")).toEqual(["hjjhjjcmnjdf"])
})

const reducesright = require ('./am_iterators_edgar.js')
