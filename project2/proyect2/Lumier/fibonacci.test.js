const fibonacci = require('./fibonacci');


describe('Fibonacci Tests', () => {
  const output = ""
  test('expect short'), () => {
      expect(fibonacci(5)).toEqual(BigInt(5))
      expect(fibonacci(6)).toEqual(BigInt(8))
  }
   test('expect looong', () => {
      let fibo2000 = fibonacci(2000)
      expect(fibonacci(2000)).toBe(BigInt(output));
  });
});
describe("some data types and different inputs", () =>{
  test("data type",() => {
    expect(typeof fibonacci(9)).toBe('bigint')
  });
  test("fibonacci with 9 input",() =>{
    expect(fibonacci(9)).toEqual(BigInt(54))
  });
  test("string number input",()=>{
    expect(fibonacci("9")).toEqual(BigInt(54))
  })
});