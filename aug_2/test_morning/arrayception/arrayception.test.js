...
test('posicion 0 es diferente a posicion 0 0', () => {
  const result = arrayGenerator(8);
  const array0th = result[0]
  const array1th = result[0][0]
  expect(array0th == array1th).toBe(false);
});

test('Second element is not an array', () => {
  const result = arrayGenerator(8);
