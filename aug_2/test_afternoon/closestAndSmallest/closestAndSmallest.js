...
function closestAndSmallest(str) {
  ...
  w = [];
  str.forEach( (elemento, i, array) => w[i] = elemento.split('').reduce(function(acumulador, elemento){
    return Number(acumulador) + Number(elemento); // Método forEach para calcular el 'weight' de cada número de la cadena  y los almacena en el arreglo w
  }, 0) );

  wsorted = w.slice(); // Copia del arreglo w ('weight' de cada número)
  wsorted.sort( (a,b)=>a-b ); // Arreglo copia con los weight's de menor a mayor

  index = [];
  for (let i = 0; i < w.length; i++) {
    for (let j = 0; j < w.length; j++) {
      if (wsorted[i] == w[j]) {
        index[i] = j;  // Indice asociado al arreglo 'w' de los elementos de 'wsorted'
      }
    } 
  }
